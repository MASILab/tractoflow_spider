...
/nextflow /tractoflow/main.nf \
	...
	--pft_seeding_mask_type interface --pft_nbr_seeds ${PFT_SEED} --run_local_tracking  --local_algo ${ALGO} \
...
xvfb-run -a --server-num=$((65536+$$)) --server-args="-screen 0 1600x1280x24 -ac" \
	scil_visualize_fodf.py results/me/FODF_Metrics/me__fodf.nii.gz --axis_name axial --output axial.png --silent \
	--background results/me/DTI_Metrics/me__fa.nii.gz --win_dims 2560 2560
xvfb-run -a --server-num=$((65536+$$)) --server-args="-screen 0 1600x1280x24 -ac" \
	scil_visualize_fodf.py results/me/FODF_Metrics/me__fodf.nii.gz --axis_name coronal --output coronal.png --silent \
	--background results/me/DTI_Metrics/me__fa.nii.gz --win_dims 2560 2560

/CODE/generate_tractoflow_spider_pdf.py ${N_SUBJ}_${N_SESS} 
...
cp -L results/*/*__local.trk results/*/*__pft.trk ${OUT_DIR}/